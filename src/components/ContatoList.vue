    ...
    <template v-if="contatos.length">
      <ion-item-sliding v-for="contato in contatos" :key="contato.id">
        ...
        <ion-item-options side="end">
          ...
          <ion-item-option color="danger" @click="confirmarExclusao(contato)">Excluir</ion-item-option>
        </ion-item-options>
      </ion-item-sliding>
    </template> <ion-item v-else>
...
const editInputs = computed(() => [
    ...
    {name: 'telefone', type: 'telt', placeholder: 'Telefone', value: editAlert.value.data.telefone}
])
